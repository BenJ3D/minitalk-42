...
/*   Updated: 2022/03/09 19:19:05 by bducrocq         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void sig_handlerold(int	signal)
{
	if (signal == SIGUSR1)
		g_tu.str[g_tu.i] = '0';
	if (signal == SIGUSR2)
...
int	main()
{
	...
	signal(SIGUSR1, sig_handlerold);
	signal(SIGUSR2, sig_handlerold);
	while(1)
	{
		pause();
		...
		if (g_tu.progress == START_RECEIVE_MSG && g_tu.bin == BINARY_OK_FOR_CHAR)
		{
			...
			{
				g_tu.msg[g_tu.y] = '\0';
				//ft_putstr_fd(g_tu.msg, 1); //affiche le message une fois complet
				free(g_tu.msg);
				g_tu.progress = STEP_PARAMETER;
				g_tu.bin = BINARY_WAIT;
